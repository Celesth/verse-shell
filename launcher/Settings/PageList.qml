...
Item {
    ...
    // into verse/custom-pages (gitignored, since it's
    ...
    Process {
        ...
        onExited: exitCode => {
            ...
            if (Settings.alertEnabled("actions"))
                Quickshell.execDetached(["notify-send", "-a", "verse", "-i", "user-trash", "Page moved to trash", trashedLabel]);
        }
    }
    ...
    SettingHint {
        ...
        anchors.topMargin: 2
        content: "drag to reorder · swipe right to delete · folders with a main.qml placed in verse/custom-pages appear here"
    }
}
