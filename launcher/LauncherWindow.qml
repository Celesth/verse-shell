...
PanelWindow {
    ...
    // focused, so `verse toggle` opens on the monitor being used rather than
    ...
    function resetState(targetPane: string): void {
        ...
        //
        // `verse toggle <page>` passes targetPane so a closed launcher opens
    ...
    WlrLayershell.namespace: "verse-launcher"
    ...
    function commitWallpaper(wall) {
        ...
        if (Settings.alertEnabled("actions"))
            Quickshell.execDetached(["notify-send", "-a", "verse", "-i", "preferences-desktop-wallpaper",
    ...
    function expandClip(clip): void {
        ...
        clipCopy.command = ["bash", "-c", `
            ...
            if ! command -v wl-copy >/dev/null 2>&1; then
                [ "$5" = "1" ] && notify-send -a verse -i system-software-install "wl-copy not found" "wl-copy (wl-clipboard) is used to place clipboard history entries back on the clipboard - install it to copy from this page."
                exit 0
            fi
            tmp=$(mktemp)
            ...
            if [ "$6" = "1" ]; then
                if [ "$2" = "img" ] && [ -s "$3/$1.png" ]; then
                    notify-send -a verse -i edit-copy -h "string:image-path:$3/$1.png" "Copied to clipboard" "$body"
                else
                    notify-send -a verse -i edit-copy "Copied to clipboard" "$body"
                fi
            fi
            sleep 0.3
    ...
    TextInput {
        ...
        // now that a page can read verse.textInput (see PageContext)
