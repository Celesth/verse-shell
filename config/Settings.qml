...
JsonAdapter {
    ...
    function alertEnabled(name: string): bool {
        return (settings.verseAlerts ?? {})[name] !== false;
    }
    ...
    // gates notify-send calls verse sends on its own behalf, split by kind:
    ...
    property var verseAlerts: Defaults.verseAlerts

    // discharging battery % the low-battery alert fires at; gated by the
    // "battery" chip in verseAlerts above (heal() clamps to 5-50)
    ...
    // how many of the most recent cached notifications `verse replay` can step
    ...
    function heal(): void {
        ...
        // verseAlerts (errors/system/battery)
        if (settings.flyouts && Object.prototype.hasOwnProperty.call(settings.flyouts, "alerts")) {
            ...
            settings.verseAlerts = { errors: wasOn, system: wasOn, battery: wasOn };
            const fly = Object.assign({}, settings.flyouts);
            delete fly.alerts;
            settings.flyouts = fly;
            settings.save();
        }
        // verseAlerts' "system" category renamed "actions"
        if (settings.verseAlerts && Object.prototype.hasOwnProperty.call(settings.verseAlerts, "system")) {
            const alerts = Object.assign({}, settings.verseAlerts);
