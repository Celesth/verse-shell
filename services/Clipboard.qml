                        ...
                        for id in "$@"; do
                            ...
                            if command -v magick >/dev/null; then
                                ...
                                if [ "$warned" = "0" ] && [ "$alerts" = "1" ]; then
                                    warned=1
                                    notify-send -a verse -i system-software-install "magick not found" "ImageMagick is used to downscale clipboard image thumbnails - install it to keep memory/decode cost down for large screenshots."
                                fi
                                cp "$tmp" "$dir/$id.png"
                            fi
                            rm -f "$tmp"
