...
// Things verse learns about the machine it's running on, plus the paths it
...
Singleton {
    ...
    // Single persistent, self-cleaning cache root for everything verse
    ...
    readonly property string cacheRoot: (Quickshell.env("XDG_CACHE_HOME") || (Quickshell.env("HOME") + "/.cache")) + "/verse"

    // scratch file for the notification tint's icon-grab round trip (see
    // NotificationFlyout's tint Canvas)
    readonly property string tintGrabPath: (Quickshell.env("XDG_RUNTIME_DIR") || "/tmp") + "/verse-tint.png"
    ...
    Process {
        ...
        command: ["bash", "-c", `
            ...
            shell_id="$3"
            crashdir="$4"
            echo "verse debug info -- $(date -Iseconds)"
