...
// User-supplied extra pages, discovered from verse/custom-pages - gitignored,
...
Singleton {
    ...
    // custom-pages behind verse's back still arrives unchecked. Sticky until
            ...
            onStreamFinished: {
                ...
                if (JSON.stringify(merged) !== JSON.stringify(existing)) {
                    ...
                    if (goodAdded.length && Settings.alertEnabled("actions")) {
                        ...
                        Quickshell.execDetached(["notify-send", "-a", "verse", "-i", "list-add", "New custom page found", body]);
                    }
                    for (const u of brokenAdded)
                        Notifier.error("Custom page “" + u.label + "” is missing main.qml", "Folders in verse/custom-pages need a main.qml entry point - see Settings > Pages.");
                }
            }
        }
    }
