...
// Everything about notifications *as verse sees them*: the alerts verse
// raises on its own behalf, the shared derivation the flyout and the replay
// cache both render from, and `verse replay` itself.
//
// verse is its own notification server, so its alerts are ordinary
...
Singleton {
    ...
    // verse-raised error reads the same at a glance. Auto-dismisses like any
    // other verse alert, on the flyout's normal Settings.notifTimeout (no -t
    ...
    function error(summary: string, body: string): void {
        if (!Settings.alertEnabled("errors"))
            return;
        Quickshell.execDetached(["notify-send", "-a", "verse", "-i", "dialog-error", summary, body]);
    }
    ...
    // is an environment gap, not something verse itself failed at.
    function missingDependency(summary: string, body: string): void {
        if (!Settings.alertEnabled("missingDeps"))
            return;
        Quickshell.execDetached(["notify-send", "-a", "verse", "-i", "system-software-install", summary, body]);
    }
    ...
    function action(icon: string, summary: string, body: string, imagePath: string): void {
        if (!Settings.alertEnabled("actions"))
            return;
        const args = ["notify-send", "-a", "verse", "-i", icon];
        if (imagePath)
    ...
    // cache. Matches on the *raw* freedesktop icon name verse itself passed via
    ...
    // Whether this is one of verse's own error/missing-dependency alerts, the
    ...
    // verse alert would expire the one before it and the user would see
    // whichever landed last. Matches the raw icon name verse passed to
    ...
    function viewOf(n): var {
        ...
        // notify-send-style call verse itself makes, and the real name only
        ...
        // verse replay (see Notifier.fireReplay) always sends "REPLAY" as its own
        ...
        if (appName === "REPLAY") {
            const originalTimestamp = Number(n.hints?.["x-verse-orig-ts"] ?? 0);
        ...
        return {
            ...
            // a replay of verse's own alert would otherwise stop counting as
            ...
            own: n.appName === "verse" || entryId === "verse",
    ...
    // verse-private hint (no standard notify-send hint carries this) so that
    ...
    function fireReplay(item): void {
        ...
        if (item.timestamp)
            args.push("-h", "string:x-verse-orig-ts:" + item.timestamp);
        if (item.appIcon)
    ...
    // Cursor into NotifCache.items (0 = most recent) that each `verse replay`
