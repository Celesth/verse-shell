...
// verse hands one of these to every custom page's root item, as a
// `verse` property, if the page declares one. Every member below is
...
// below are promised to keep working after a verse update; anything
...
//
// No icon glyphs here - verse's own icon codepoints (which glyph means
// what) are an internal detail of how verse draws itself, not
// something a page should depend on. A page that wants icons uses
// `iconFont` below (verse's own vendored font, already loaded) or
...
//
// A page sizes itself: verse just centers a window around whatever
// width/height the page reports (420x320 if it reports none) - nothing
// clips, so a page that's too big just overlaps.
//
// Pages can also hand something back to verse, the other direction:
...
QtObject {
    ...
    // Family name of verse's own icon font. It's already loaded once by
    // verse itself, so using this instead of your own FontLoader saves
    ...
    // it's off. Bind your corners through it - `radius: verse.radius(8)` -
    ...
    // from verse. If you want to run code when it changes, mirror it onto
    // your own root item first, the same way launcherOpen is used below:
    //   readonly property bool pageActive: verse ? verse.pageActive : false
    //   onPageActiveChanged: ...
    ...
    // Whatever the user's currently typed into verse's hidden search box.
    ...
    // Gives keyboard focus back to verse's hidden search box. Only call
    // this if your page grabbed focus itself (e.g. put its own TextInput in
    // front and called forceActiveFocus() on it) - verse's own keybinds,
    ...
    // it once per item, any time you want it to (re)play - verse owns the
    ...
    function tileIn(item, slot, cols) {
        ...
        if (!rec) {
            ...
            const spring = tileSpringFactory.createObject(item, { verseItem: item, verseOffset: offset, verseSlot: s, verseCols: c });
            rec = { item, spring };
            tileRegistry.push(rec);
        } else {
            rec.spring.verseSlot = s;
            rec.spring.verseCols = c;
        }
        rec.spring.restart();
    }
    // tileIn()'s text half: hands back `source` as it looks partway through
    // the scramble verse's own labels play as a page opens, so your text
    ...
    //   Text { text: verse.scramble(modelData.name, index, 3) }
    ...
    // slot/cols for a label on a tile and the two land together: verse's own
    ...
    readonly property Component tileSpringFactory: Component {
        SequentialAnimation {
            id: spring
            property var verseItem: null
            property var verseOffset: null
            property int verseSlot: 0
            property int verseCols: 1
            PropertyAction { target: spring.verseItem; property: "opacity"; value: 0 }
            PropertyAction { target: spring.verseItem; property: "scale"; value: Anim.fromScale }
            PropertyAction { target: spring.verseOffset; property: "y"; value: Anim.fromY }
            PauseAnimation { duration: Anim.stagger(spring.verseSlot, spring.verseCols, 60) }
            ParallelAnimation {
                NumberAnimation { target: spring.verseItem; property: "opacity"; to: 1; duration: Anim.fadeDuration; easing.type: Easing.OutCubic }
                NumberAnimation { target: spring.verseItem; property: "scale"; to: 1; duration: Anim.duration; easing.type: Anim.easing; easing.overshoot: 2.2 }
                NumberAnimation { target: spring.verseOffset; property: "y"; to: 0; duration: Anim.duration; easing.type: Anim.easing; easing.overshoot: 2.2 }
            }
        }
    }
